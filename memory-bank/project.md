...
- **Reshuffle upcoming rounds**: a Courts-tab button ("Reshuffle upcoming rounds & opponents", hidden when finished or when no free round exists) confirm-guards a manual shuffle that reorders the free unplayed rounds among themselves and re-pairs each round's pairs into courts — keeping every pair (team) together, never duplicating a player within a round, leaving played and partially-scored rounds and all saved scores untouched, and re-sequencing round numbers (`match.reshuffleUnplayed` / `state.reshuffleUnplayed`).
