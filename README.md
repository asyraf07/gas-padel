...
- **Match type × pairing** — two independent settings instead of fixed formats:
  ...
  - **Pairing**: *Normal*, *Mixed* (one male + one female per team, gender required, and an equal number of men and women to start), or *Fixed* (roster entries are team names — Americano never re-pairs the same two teams, Mexicano pairs top vs bottom).
- **Matchmaking fairness**: court capacity (with fair byes), play-count balance, wait-time fairness, and partner/opponent uniqueness until combos are exhausted; mixed events fill courts with "mixed pair + best available" before forcing a bye, and mixed Americano never lets a partner pair repeat (round count caps at `min(men, women)`).
...
- **Reshuffle**: shuffle the order of upcoming rounds and which pairs face each other with one button (confirm-guarded) — pairs stay together, played rounds and scores are untouched.
...
- **Quick score entry**: tap either team's score to open a score-picker modal with every option from 0 to the target (0–21) — picking one auto-derives the opponent score, so ties/over-totals are impossible. A corner icon switches the modal into a **live score counter** (`−`/`+` per team, tally kept on the page when you close it, and the close button becomes **Finish** once the scores reach the target).
